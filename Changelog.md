...
- `[Blessie Faith Bongalos]` - Created `CHANGELOG.md` at the repository root following the Keep a Changelog format, structured to log individual contributions across the four project deliverables (architecture diagram, deployment documentation, cost report, and video presentation).
...
### Added
- `[Blessie Faith Bongalos]` - Added `schedule_type` ENUM('reservation', 'class') column and `idx_schedule_type` index to `eduroom_schema.sql` so the production Azure Database for MySQL Flexible Server schema would match the application's current data model upon deployment.
### Changed
- `[Blessie Faith Bongalos]` - Updated `.gitignore` to include `.env`, `ca.pem`, `venv/`, and `__pycache__/` to prevent credential and environment files from being committed to the GitHub repository connected to the Azure App Service deployment workflow.
### Fixed
- `[Blessie Faith Bongalos]` - Resolved merge conflict on `.gitignore` between the `local-demo` and `main` branches, then merged the feature code into `main` so the latest application state would be picked up by the GitHub Actions deployment pipeline targeting Azure App Service.
### Removed
- `[Blessie Faith Bongalos]` - Removed `.env` (database connection credentials) and `ca.pem` (SSL certificate) from Git tracking after discovering they had been committed to the `local-demo` branch. Removal was required before migrating secret handling to Azure Key Vault with Managed Identity authentication.
...
- `[Blessie Faith Bongalos]` - Reviewed the three cloud optimization callouts on the architecture diagram (GitHub Actions CI/CD, multi-instance App Service Plan, and Managed Identity for Key Vault) and confirmed each maps to a distinct optimization category required by Section 6 of the project rubric.
